 ...
 */
export async function runWorkflowEngine(message: any) {
    const userPhone = message.from;
    
    // 1. Obtener el estado actual del usuario/oportunidad
    const stateRef = db.collection('opportunityStates').doc(userPhone);
    ...
    const nextStepRef = db.collection(`workflows/${workflowId}/steps`).doc(currentState);
    const nextStepSnap = await nextStepRef.get();

    if (!nextStepSnap.exists) {
        logger.warn(`No se encontró el paso '${currentState}' en el workflow '${workflowId}'. Finalizando.`);
        return;
    }
